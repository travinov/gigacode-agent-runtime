...
Знак `?` рядом с каждым управляемым полем открывает краткую подсказку:
назначение, допустимые значения, пример и актуальные элементы локального
каталога. Раздел `Описание` содержит подробные пошаговые инструкции по
настройке Runtime, созданию agents, Skills и сценариев, последовательным и
параллельным DAG, loops, permissions, Preview и Apply. Справка входит в
офлайн-пакет и не загружает внешние ресурсы.
